...
When running multiple strategies together, `MAX_POSITION_VALUE` and `MAX_OPEN_POSITIONS`
remain the account-wide defaults, and `TRADE_COOLDOWN_SECONDS` remains the default symbol
cooldown. Optional strategy-specific overrides can be set when one strategy needs different
sizing, book size, or pacing:

- `STOCH_MACD_MAX_POSITION_VALUE` / `STOCH_MACD_MAX_OPEN_POSITIONS` / `STOCH_MACD_TRADE_COOLDOWN_SECONDS`
- `MACD_MAX_POSITION_VALUE` / `MACD_MAX_OPEN_POSITIONS` / `MACD_TRADE_COOLDOWN_SECONDS`
- `STEADY_INTRADAY_MAX_POSITION_VALUE` / `STEADY_INTRADAY_MAX_OPEN_POSITIONS` / `STEADY_INTRADAY_TRADE_COOLDOWN_SECONDS`
- `OPENING_IMPULSE_MAX_POSITION_VALUE` / `OPENING_IMPULSE_MAX_OPEN_POSITIONS`
- `GAP_AND_GO_MAX_POSITION_VALUE` / `GAP_AND_GO_MAX_OPEN_POSITIONS`
- `SPIKE_MAX_POSITION_VALUE` / `SPIKE_MAX_OPEN_POSITIONS`
- `MAHA7_MAX_POSITION_VALUE` / `MAHA7_MAX_OPEN_POSITIONS`
